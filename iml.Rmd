...
do.call( rbind, vip_flat) %>% as.data.frame %>%
  ...
  ggthemes::scale_fill_fivethirtyeight() + ggtitle("Permutation Importance") + theme(text = element_text(family="Serif"))
```
...
rf_pdp %>% ggplot(aes(color = variable, y =  avg_pred, x = value)) + 
  ...
  theme_fivethirtyeight() + scale_color_hc()+ theme(text = element_text(family="Serif"))
```
...
pdp_imps%>%  ggplot(aes(x = variable, y = importance, fill = model)) + geom_bar(stat = "identity") +
  facet_grid(model ~ ., scales = "free") + coord_flip() + ggthemes::theme_fivethirtyeight() + 
  ggthemes::scale_fill_fivethirtyeight() + ggtitle("Partial Dependence Importance")+ theme(text = element_text(family="Serif"))
```
...
correlator  <-  function(df){
	df %>%
		...
		corrplot("upper", addCoef.col = "white", number.digits = 2,
			 ...
			 order="hclust", 
			 tl.srt=45, tl.cex = 1, number.font=3)
}
X_test %>% correlator
```
...
plot_ice <- function(df) {
  ...
  plot_single_curve_set <- function(col) {
    ...
    p <<- p + geom_line(aes_string(x = "value", y = col), color = "black", alpha = 0.6) + 
      facet_wrap(variable ~ ., scales = "free")+ theme(text = element_text(family="Serif"))
  }
...
ice_imp_df %>% gather(key = "key", value = "value", -model) %>%
  ...
  ggtitle("ICE Variable Importance!")+ theme(text = element_text(family="Serif"))
```
...
cor(df)
p <-  ggplot() + geom_point(data = df, aes(x=x, y=y))+ theme(text = element_text(family="Serif"))
p
```
...
p <-  ggplot(df) + geom_point(aes(x = x, y = y)) +
  theme(panel.grid.major.y = element_blank(), 
    ...
    panel.grid.minor.x = element_blank()) + theme(text = element_text(family="Serif"))
...
p2 <- p + geom_vline(data = grid, aes(xintercept = x)) + 
      ...
      geom_label(data = labels, aes(x = xlab, y = ylab, label = labels)) + theme(text = element_text(family="Serif"))
p2
```
...
ggplot(data_grouped) + geom_point(aes(x = loc, y = y)) + 
      ...
      transition_states(state, transition_length = 1, state_length = 2)+ theme(text = element_text(family="Serif"))
```
...
plot(ALE) + theme(text = element_text(family="Serif"))
```
...
ge_gather %>% ggplot() + geom_point(aes(x =variable, y = prediction, color = variable), size = 5) +coord_flip() + scale_color_few() + ggtitle("Prediction") + theme_hc() +  transition_reveal(idx)+ theme(text = element_text(family="Serif")) 
```
